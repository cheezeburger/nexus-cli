...
# Parse out the ollama version
OLLAMA_VERSION=$(grep -m1 '"ollamaVersion"' "$CLI_METADATA_FILE" | sed -E 's/.*"ollamaVersion"[[:space:]]*:[[:space:]]*"([^\"]*)".*/\1/')

[ -z "$OLLAMA_VERSION" ] || [ "$OLLAMA_VERSION" = "null" ] && \
    error "Failed to retrieve Ollama version from $AUTO_UPDATE_LATEST_VERSION_ENDPOINT"

status "Inference runtime version: $OLLAMA_VERSION"
...
status "Downloading Inference CLI launcher..."
...
status "Downloading Inference runtime..."

if [ "$UNAME" = "Linux" ]; then
    INFERENCE_RUNTIME_URL="${BUCKET_URL}/ollama-runtimes/${ARCH}/inference-runtime-linux-${ARCH}-${OLLAMA_VERSION}"
    LIB_URL="${BUCKET_URL}/ollama-runtimes/${ARCH}/inference-linux-${ARCH}-lib-${OLLAMA_VERSION}.tar.gz"
...
if [ "$UNAME" = "Darwin" ] && [ "$ARCH" = "darwin-aarch64" ]; then
    INFERENCE_RUNTIME_URL="${BUCKET_URL}/ollama-runtimes/macos/inference-runtime-darwin-aarch64-${OLLAMA_VERSION}"
...
if check_gpu lspci amdgpu || check_gpu lshw amdgpu; then
    ...
    curl --fail --show-error --location --progress-bar -o $TEMP_DIR/ "$BUCKET_URL/ollama-runtimes/amd64-rocm/inference-runtime-linux-amd64-rocm-$OLLAMA_VERSION.tgz" \
